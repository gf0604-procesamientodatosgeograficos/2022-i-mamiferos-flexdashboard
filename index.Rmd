...
mamiferos <-
  ...
  filter(taxonRank == "SPECIES" |
           taxonRank == "SUBSPECIES") # exclusión de registros con identificación superior a especie
...
leaflet() %>%
  ...
  addProviderTiles(provider = providers$Esri.WorldImagery, group = "Imágenes de ESRI") %>%
...
### Tabla de registros de presencia {data-width=500}

```{r resumen-tabla-registros-presencia}
# Tabla de registros de presencia

mamiferos %>%
  st_drop_geometry() %>%
  select(order, family, species, eventDate, locality) %>%
  datatable(
    colnames = c("Orden", "Familia", "Especie", "Fecha y hora", "Localidad"),
    options = list(
    pageLength = 10,
